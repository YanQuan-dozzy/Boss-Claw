...
### Q16：启用了猎聘 / 智联 / 前程无忧，但采集 / 投递不动或提示未登录？

非 BOSS 平台的**搜索采集与投递都走 Camoufox 隐身引擎**（webview 可视化采集为 BOSS 专属链路）：

- 到「自动沟通」页确认对应平台卡片显示引擎就绪且**已登录**，未登录则点「登录」扫码（Cookie 按平台持久化 `~/.bossclaw/camoufox-cookies-{平台}.json`）
- 到「设置 → 招聘平台」确认平台已勾选启用，且未把某平台从工作台采集多选里取消
- 只投 BOSS 时无需启用其他平台（设置 → 招聘平台 可整体取消勾选，但至少要保留一个启用平台）

### Q17：猎聘投递语义特殊吗？

猎聘点「聊一聊」后由平台 **App 预设招呼语自动发送**，脚本不注入文本——**请先在猎聘 App 里设置好招呼语文案**；投递成功以聊天窗打开 / 按钮变「继续聊」为准。

### Q18：智联招聘每日投递上限是多少？

智联平台侧每日约 **100 次**投递上限，默认每日投递目标即 100/日，接近上限自动停止交人工；其余平台（BOSS / 猎聘 / 前程无忧）默认 120/日。各平台独立计数、互不挤占，可在「设置 → 招聘平台」逐平台调整（0 = 不限，仍受平台侧上限与安全上限收窄）。
