...
const instructions = [
  'BossClaw 项目操作 MCP —— 让 agent 能够构建、运行、诊断并驱动 BossClaw 桌面应用。',
  '',
  `仓库根：${REPO_ROOT}`,
  `应用目录：${DESKTOP_DIR}`,
  ...
  '',
  '推荐工作流：',
  '  1) bossclaw_guidelines（先读约束与安全不变量）→ bossclaw_project_info（建立全局认知）',
  '  2) 定位代码：bossclaw_search / bossclaw_read_file；跨进程结构：bossclaw_ipc_surface',
  '  3) 改动后必须 bossclaw_verify（tsc -b + vite build）；UI/主进程改动追加 bossclaw_smoke',
  '  4) 观察/驱动运行中的应用：bossclaw_app_start（默认开控制桥）→ bossclaw_app_state → bossclaw_app_action',
  '  5) 排查现场：bossclaw_logs（app/render/webview）+ bossclaw_state_summary（任务与安全状态）',
  '',
