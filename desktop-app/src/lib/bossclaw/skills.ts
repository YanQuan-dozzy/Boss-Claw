...
export const BOSS_SKILLS: BossSkill[] = [
  ...
  },
  {
    ...
    defaultEnabled: true,
    instructions:
      '评分必须稳定（同一岗位重复分析波动 ≤±5，严禁所有岗位挤在 60-70 之间）；**严格区分闸门（优先级高于其它措辞）**：match/strong 只授予「核心技能直接命中 + 方向高度同向 + 无明显影响上手的实质缺口」的岗位，凡核心职责依赖「技术栈可迁移 / 能补位 / 需现学 / 简历无任何同类擅做项目经历」才能胜任的一律不得判 match、划到 cautious（score 50-64，缺口严重可低至 40-49），「技能栈仅沾边可迁移」与「核心技能直接命中」必须拉开分数（前者 ≤64、后者才可 65+）；dimensionScores 必须输出五个固定维度（skill 技能/direction 方向/salary 薪资/education 学历/experience 经验）的 {score: 0-100 整数, evidence: 一句 ≤30 字依据}，逐维独立语义评估、可纠正本地逐词误报（技能维同义/同族已覆盖不算缺口：Git 覆盖 GitHub/GitLab、FastAPI 覆盖 Flask/Django、TypeScript/React 覆盖 JavaScript），严禁编造简历不存在的技能；技能维按 JD 要求 vs 简历真实技能栈整体覆盖度评分：**根本性技术栈错位**（岗位核心语言/框架与简历完全不同族且无法迁移，如岗位要 C++/C#/Rust/Go/嵌入式而简历只有 Java/JavaScript/Python 业务开发，或岗位要求精通某语言而简历任何技能/经历/项目均未使用）→ ≤25 且**整体档位不得高于谨慎**，同族可迁移（Python↔Java、React↔Vue、MySQL↔PostgreSQL）→ 50-70，核心技能直接命中 → 85+，部分缺失按缺口严重度扣分、表达缺口从轻；薪资维度一律以本地校准信息的岗位薪资及折算值为准（明显低于期望下限 ≤30 / 达到或高于期望 90-100 / 部分重叠按比例 / 期望未设或面议 60-70）；学历达标 100 / 高于要求 90 / 不达标 ≤30 / 未要求 70；经验按年限比例 ≥1.0→100 / 0.8+→85 / 0.6+→65 / 0.4+→45 / 以下→30 / 未要求 65；信息不足给中性分并在 evidence 写明原因；五维加权（34%/28%/14%/8%/6%）只参与总分档内微调、不得改变档位（唯一例外：技能维 ≤25 强制档位不高于谨慎）；硬性条件（学历/经验年限/地点/求职类型）不满足记入 hardBlocks 且 score ≤35；matchedEvidence 写 3-6 条「岗位要求 X — 简历具备 Y」的证据句（每条 ≤40 字，只写真实可核验的匹配点，禁止「沟通能力强」等无证据泛词）；gaps 只列岗位明确要求、简历与画像均未涉及的实质技能，每条格式「技能名：匹配说明」（技能名 ≤20 字、匹配说明 20-80 字），匹配说明用求职者看得懂的自然语言写清「岗位场景要求 + 简历缺口现状 + 一句补强建议」三要素，禁止多条说明雷同、禁止空话；简历做过但未写清的可标「表达缺口：技能名」，同义/同族已覆盖不计缺口、非技能词（HR/bug/Demo/JD）不计缺口，无实质缺口留空数组、严禁凑数；严禁把招聘方在线/活跃状态或岗位发布时间当作匹配理由；greeting 必须求职者第一人称（"您好，我想应聘贵公司的{岗位名}"开头），**项目亮点精炼**：只挑 1-2 个与岗位最相关的真实项目/技能并一句话概括「做了什么 + 关键成果/最贴岗位的技术要点」，禁止罗列技术栈清单、不写无关经历；全文不超过 250 字、单行不换行，只引用简历事实，严禁招聘方口吻，不得承诺薪资、到岗时间、面试时间。',
  },
  {
    id: 'greetings',
    name: '打招呼语（工作台定制）',
